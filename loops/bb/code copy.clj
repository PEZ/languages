(defn main []
  (let [u (parse-long (first *command-line-args*)) ; Get an input number from the command line
        r (rand-int 10000) ; Get a random number 0 <= r < 10k
...
(comment
  (time (binding [*command-line-args* ["40"]]
          (main)))
  (main)
  ((vec  (range 100)) 50)
  :rcf)
